		...
		struct device_attribute *attr, char *buf)
{
	...
	attn_state = gpio_get_value(rmidev->rmi4_data->irq_gpio);
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = gpio_export(rmi4_data->irq_gpio, false);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
...
{
	synaptics_rmi4_new_function_s1302(RMI_DEV, true,
			rmidev_init_device,
...
{
	synaptics_rmi4_new_function_s1302(RMI_DEV, false,
			rmidev_init_device,
			rmidev_remove_device,
			NULL);
