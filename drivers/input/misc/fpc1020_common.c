...
//Changhua.Li@Prd.BSP, 2015-02-4 Remove for not spi access in AP
	...
	if (error < 0){
	    if(error == -EIO && (fpc1020->to_power == true)){//IC is out of state,just power reset(GPIO41)
            dev_err(&fpc1020->spi->dev, "%s,fpc1150 IC is out of state,power off!!!\n", __func__);
	        gpio_set_value(fpc1020->vdden_gpio,0);
        	mdelay(3);
        	gpio_set_value(fpc1020->vdden_gpio,1);
        	mdelay(3);
        	dev_err(&fpc1020->spi->dev, "%s,fpc1150 IC is out of state,power on done!!!\n", __func__);
	        fpc1020_gpio_reset(fpc1020);
